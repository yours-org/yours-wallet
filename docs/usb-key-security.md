...
The secret on the drive is a plain file and can be copied. The drive is a possession factor guarded by the password, not a tamper-resistant one. Product copy must say "USB unlock" or "USB key," never "hardware wallet."
...
- **Offscreen keeper** (`offscreen.html`, `src/offscreen.ts`, the `offscreen` manifest permission). Chrome revokes a File System Access grant when the last extension page closes, and the action popup closes on any focus change. The keeper is an invisible extension page the background opens whenever USB unlock is on, so the grant obtained once (the "Allow USB access" click) lasts the whole browser session and later popup opens go straight to the wallet. It also probes the drive every 3 s and reports present/absent to the background, which locks the wallet 5 s after the key goes missing and refuses dApp spend/sign/reveal calls with "Insert your USB key to continue" in the meantime. It holds no secrets beyond the momentary unwrap that verifies the key, and reports only a state word. Without it: one allow click per popup open, no background lock-on-removal, and provider reads keep working with the key out. It was removed and then restored on 2026-09-10 for those reasons.
