...
**Encryption.** Everything except `restore.json` is AES-GCM under `HKDF(passKey, "yours-usb-backup-v1")`. Restore rebuilds that key from the drive's own key file plus the password: stick secret → unwrap master (wrappers in `restore.json`) → combined passKey → backup key. So the drive plus the password restores; the drive alone reveals nothing.
