...
- **Presence policy** (`src/services/usbPresence.ts`): the popup's wallet is wrapped so spend/sign/reveal calls probe first; the prompt window probes before rendering a request; two consecutive misses lock. Read-only calls are not gated. Only pages can read the drive, so removal is noticed when the popup or a prompt window next checks, not in the background.
- **Drive grants do not outlive the popup.** Chrome revokes a File System Access grant when the last extension page closes, and the action popup closes on any focus change. So every unlock from the popup is two clicks: "Allow USB access" (Chrome's bubble, which needs a gesture) then the password. An offscreen keeper page would make the grant last the whole browser session, but it needs the `offscreen` manifest permission, which was deliberately left out. The standalone USB window is the fallback if Chrome closes the popup under its bubble, and the only place the folder picker can run.
